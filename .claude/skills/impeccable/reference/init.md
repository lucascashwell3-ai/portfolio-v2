...
Also check Lucas's own design history so a new brand-register project doesn't accidentally repeat his
last few font/accent/aesthetic-lane choices — a different failure mode than the internet-wide
monoculture the reflex-reject lists in brand.md already cover:

```bash
node .claude/skills/impeccable/scripts/design-history.mjs read
```

Treat the output as one more input alongside brand.md's reflex-reject lists when Step 3/4 confirm a
direction, not a hard rule. If the script reports no history found, continue without it — never block
on this.
...
If this session wrote (or already found) both a `brand`-register PRODUCT.md and a DESIGN.md with a
concrete font/accent/aesthetic-lane decision, record it:

```bash
node .claude/skills/impeccable/scripts/design-history.mjs append --project "<project name>" --font "<font family>" --accent "<short color/strategy description>" --lane "<aesthetic lane name, e.g. from brand.md's reflex-reject-lanes vocabulary>"
```

Skip silently for `product`-register projects (the history log tracks brand-surface differentiation,
not app UI) or when DESIGN.md wasn't produced this run — a later `/impeccable document` run won't
trigger this automatically; that's a known gap, not a bug.
