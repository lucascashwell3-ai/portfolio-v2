...
- Timing budget: buttons 100–160ms, tooltips/popovers 125–250ms, modals/drawers 200–500ms. Exit should usually be faster than enter (system responding vs. user deciding).
- Never animate from `scale(0)` — nothing in the real world appears from nothing. Start from `scale(0.9–0.95)` combined with opacity.
- CSS transitions (not `@keyframes`) for anything that can be triggered rapidly or interrupted mid-animation — transitions retarget smoothly, keyframes restart from zero.
- Framer Motion's `x`/`y`/`scale` shorthand is **not** hardware-accelerated (runs on the main thread via `requestAnimationFrame`); use the full `transform` string when it matters under load.
- Gate hover animations behind `@media (hover: hover) and (pointer: fine)` — touch devices flash hover state on tap otherwise.

#### Accessibility

- Disabled buttons should not carry tooltips (unreachable for keyboard users — they're out of tab order).
- Focus rings: use `box-shadow`, not `outline` (outline ignores border-radius in some browsers).
...
- Clicking an input's label should focus the input; wrap inputs in `<form>` so Enter submits.
- Toggles take effect immediately — no confirmation step.
- No dead zones between items in a list — extend padding, don't leave gaps.
- Scale values should be proportional to trigger size: don't scale a pressed button from 1 → 0.8, use ~0.96; don't scale a dialog in from 0 → 1, fade+scale from ~0.8.
- Popovers scale in from their trigger (`transform-origin` at the trigger), not from center — modals are the exception, they stay centered.
